...
\textbf{LOCAL hierarchy of problem classes:} let $\mathcal{A},\mathcal{B}$ be classes of
LOCAL problems. We say that $\mathcal{A}\into \mathcal{B}$ if there exists an injective function
$\tau:\mathcal{A}\to\mathcal{B}$ such that $\forall \Pi_A\in\mathcal{A}\ \Pi_A$ has the
same asymptotic locality as $\tau(\Pi_A)$.

\textbf{LOCAL equivalence of problem classes:} let $\mathcal{A},\mathcal{B}$ be classes of
LOCAL problems. We say that $\mathcal{A}$ is equivalent to $\mathcal{B}$ (denoted
$\mathcal{A}\cong \mathcal{B}$) if $\mathcal{A}\into \mathcal{B}$ and $\mathcal{B}\into\mathcal{A}$.
...
\begin{proof}
    ...
    \begin{itemize}
        \item We extend $\Sigma\uout$ by an input error label $I_E$, a leaf label $I_L$,
        and labels $I_i$ indexed by the input labels; formally,
        $$\Sigma\uout'=\Sigma\uout\cup\Set{I_E,I_L}\cup\Set{I_i\dv i\in\Sigma\uin}$$
        \item We define $\Nc'$ in four steps:
        \begin{enumerate}
            ...
            $\geq 4$ by appending leaf labels $I_L$ to both the requirement and the
            filler: formally,
            $$\Nc'_2=\Set{(,F\cup\MSet{I_L})\dv (R,F)\in\Nc'_1} $$
            \item We allow nodes of degree $\geq 4$ who receive at least one error
        \end{enumerate}
