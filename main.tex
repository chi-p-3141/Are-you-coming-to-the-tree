...
\end{thm}
\begin{proof}
    Let $\Pi=(\Sigma\uout,\Nc,\Ec)$. We will define a procedure we call $Simplify(\Pi)$, which will either return `SU' for
    strongly unsolvable or a node edge checkable problem $\Pi_1$ with the same solvability
    horizon as $\Pi$. We will define the auxiliary prodcedure $SimplifyStep(\Pi)$ first:
    \begin{enumerate}
        \item We turn $\Pi$ into a node-edge checkable problem $\Pi'$ without inputs by replacing $\Nc$ with $\Nc'$, where
        $$\Nc'=\Set{R\cup P\dv (R,F)\in \Nc, P\subseteq F}.$$
        In other words, $\Nc'$ contains the requirement from every configuration of $\Nc$ and
        ...
        classes} of $\Pi'$; by running a BFS search on it, we can find which classes are
        \emph{possible} (=can be reached by a B-hyperpath from the starting nodes) and
        which are \emph{infinitely possible} (=can be reached by a B-hyperpath which
        contains a loop from the starting nodes).
        \item We find the \emph{constant set} $\bigcap\Cc$ of the family $\Cc$, meaning
        $$\bigcap\Cc:=\Set{x\in\Sigma\uout\dv\forall C\in\Cc, x\in C}.$$
        \item 
    \end{enumerate}
\end{proof}
