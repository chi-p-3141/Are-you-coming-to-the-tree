...
the \emph{solvability horizon} of a weakly solvable problem. We say that a problem is
...
\end{thm}
\begin{proof}
    We include the definitions of B-hypergraph, simple hyperpath and B-hyperpath from
    \cite[Chapter 2]{thakurLinearConnectivityProblems2009} and \cite[Definition 2.4]{ausielloOptimalTraversalDirected1997}

//TODO copy them

    Let $\Pi=(\Delta,\Sigma\uin,\Sigma\uout,\Nc,\Ec)$, we show that there is a way 
    to compute the set $\frCl(\Tc_{1,\Delta})$ for $\Pi$ in
    finite time. To do so, we construct the \emph{existential graph of classes} of $\Pi$,
    which is a directed B-hypergraph we denote with $G^{Cl}_{\Pi}$ and define as follows:
    \begin{itemize}
        \item the set of nodes of $G^{Cl}_{\Pi}$ is $\Pow(\Sigma\uout)$,
        \item there is an edge from $\Set{Cl_1,\ldots,Cl_k}$ to $Cl$ if and only if one of
        the following applies:
        \begin{enumerate}[(A)]
            \item there is a hanging tree of class $Cl$ whose set of sub-hanging trees
            (meaning, the hanging trees obtained by removing the hook node and all of its
            adjacent half-edges) is exactly $\Set{Cl_1,\ldots,Cl_k}$, or
            \item $k=2,Cl=\emptyset$ and $(Cl_1\times Cl_2)\cap \Ec = \emptyset$.
        \end{enumerate}
        \item Additionally, we mark the nodes corresponding to
    ...
    this hypergraph in finite time $[\bigstar 1]$, and we show that we can compute the solvability
    horizon of $\Pi$ by traversing it $[\bigstar 2]$.
    ...
    $\Set{\Nc_{1,\chi}}_{\chi\in\Sigma\uin}$; then we iterate over all the ornaments
    of depth 1 (ie containing only the hook node and leaves) and max degree $\Delta$, over
    ...
    add the class to a set $\mathcal{U}$ and add an edge in $G^{Cl}_{\Pi}$ from the labels that appear on to the node corresponding
    ...
    To show that this algorithm is correct, observe that //TODO finish: it's a simple
    inductive argument on the maxdepth.
...
% automata based definition for thm 4.1, removed since I'm not really using anything that
% I couldn/t describe in graph language.

We do so by constructing a nondeterministic finite semiautomaton $\Mc_{\Pi}$
    over the unary alphabet $\Sigma=\Set{\epsilon}$ following roughly similar steps to
    \cite{changDistributedGraphProblems2023}. Explicitly, we construct $\Mc_{\Pi}$ as
    follows:
    \begin{itemize}
        \item The set of states is $\Pow(\Sigma\uout)$.
        \item There is a transition from $Cl_1$ to $Cl_2$ if there is a hanging tree of class
        $Cl_2$ containing a hanging tree of class $Cl_1$ as a child of the hinge node.
        %this might need a diagram
        \item The only starting state is the set of all possible labels that a leaf will accept.
        \item The automaton has no accepting state.
    \end{itemize}    


%star/filling boxes problem leaf/branch label for a constraint hanging tree types

\textbf{Class certificate:} given a node-edge checkable problem or FSL problem $\Pi$ and a
class $Cl\subseteq\Sigma\uout$, we want to classify classes based on how 

\textbf{Class set:} given a node-edge checkable problem or FSL problem $\Pi$, we define
its \emph{class set} as
$$\frCl(\Pi):=\Set{(Cl,b,\Tc)\dv Cl\subseteq\Sigma\uout,b=\begin{cases}0&\text{if }\end{cases}} $$
...
\begin{proof}
    ...
    as:
    \begin{itemize}
        \item $\Sigma\uout'=\Sigma\uout\cup \Sigma_I$, where $\Sigma_I:=\Set{(I,n)\dv
        n\in\Sigma\uin}\cup\Set{(I,L),(I,Err),Err}$,
