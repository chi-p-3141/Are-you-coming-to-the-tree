...
\end{itemize}
\subsubsection{Common and uncommon assumptions}
...
is strongly unsolvable, we define it as $\infty$. We denote the solvaility horizon of
$\Pi$ as $S_h(\Pi)$.
...
\subsubsection{New-ish definitions}
...
\subsubsection{Actually new definitions}
...
\end{enumerate}
\subsection{List of possible results}
\begin{enumerate}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \begin{enumerate}
            ...
            \begin{align*}
                ...
                &\cup\Bigl\{(\MSet{I_i,I_{i+1}},\emptyset)\Bigm| i\in\Set{1,\ldots,\abs{\Sigma\uin}-1}\Bigr\}\cup\Set{(\MSet{I_{\abs{\Sigma\uin}},I_E},\emptyset)}
            \end{align*}
        ...
        $$\Ec'=\Ec\cup\Set{(I_E,I_E),(I_L,I_L),(I_P,I_P)}\cup\Set{(I_i,I_i)\dv
    ...
    $\tau(\Pi)\to\Pi$: assume we have an algorithm $\Ac$ to solve $\tau(\Pi)$ in $T(n)$
    rounds. Let $(G=(V,E),l)$ be a $\Sigma\uin$-labelled graph with $n$ nodes, on which we
    want to solve the problem $\Pi$; we construct a graph $G'$ as follows: first, append leaves to every node of the graph until every original
    node has degree $\geq 2$; then append to each original node $v\in V$ a path of length $l(v)+1$, representing the
    input label. Since we have appended at most $1+\abs{\Sigma\uin}+1$ new nodes to each
    node $v$, this new graph has $\leq (\abs{\Sigma\uin}+3)n\in O(n)$ nodes. We apply $\Ac$ to
    this new graph; we will now prove that if we restrict the output labelling of $\Ac$ to
    the edges and nodes of the original $G$, we obtain a solution to $\Pi$. 

    Now, assume that $\Pi$ is strongly unsolvable.
\end{proof}
...
whether $\Pi$ is strongly unsolvable or weakly solvable on trees and compute $S_h(\Pi)$. %in what time?
\end{thm}
\begin{proof}
    ...
    strongly unsolvable and $S_h(\Pi)=\infty$. We assume this is not the case.
    ...
    $[\bigstar 2]$ We claim that the solvability horizon of $\Pi$ corresponds to the
    ...
    large depth, and $\Pi$ is strongly unsolvable (so $S_h(\Pi)=\infty$). Similarly, if no hyperpaths exist
    ...
    diameter and the solvability horizon.\footnote{We are only concerned about computability
...
whether $\Pi$ is strongly unsolvable or weakly solvable and compute $S_h(\Pi)$. %in what time?
\end{thm}
\begin{proof}
    Let $\Pi=(\Sigma\uout,\Nc,\Ec)$. We will create a procedure we call $Simplify$, which
    will return a node edge checkable problem $\Pi'$ and a value $k$ such that $S_h(\Pi)=\max\{S_h(\Pi'),k\}$. We define $Simplify(\Pi)$ as follows:
    \begin{enumerate}
        \item We turn $\Pi$ into a node-edge checkable problem $\Pi_1$ without inputs by replacing $\Nc$ with $\Nc'$, where
        $$\Nc_1=\Set{R\cup P\dv (R,F)\in \Nc, P\subseteq F}.$$
        In other words, $\Nc_1$ contains the requirement from every configuration of $\Nc$ and
        every combination of symbols in the corresponding fillers; even though we are abusing
        notation slightly with the union, $P$ is intended to be a subset (containing at most one copy of
        each filler symbol, including none).
        \item Using the procedure we outlined above, we can build the \emph{existential graph of
        classes} of $\Pi'$; we then run a BFS search on this hypergraph to compute all of
        the \emph{longest} B-hyperpaths\footnote{The length of a B-hyperpath is the same as
        the diameter of corresponding hyperpath tree.} from the starting nodes to each
        node. We denote this as $LongPath(C)$ and collect the results in a set of pairs $\Cc$, formally defined as
        $$\Cc=\Set{(C,k)\dv
        C\subseteq\Sigma\uout,k=\begin{cases}0&\text{if there is no B-hyperpath to $C$,}\\\infty&\text{if $LongPath(C)$ contains a loop,}\\\abs{LongPath(C)}&\text{
        otherwise}\end{cases}}.$$
        \item We iterate through $\Nc$, constructing sets
        $\Nc',\mathcal{F}$\footnote{Note that $\mathcal{F}$ is not part of the output of
        $Simplify$, but is useful in proving its correctness.} and $k$ as
        follows: %make this pseudocode
        initialise $m=0,\Nc'=\mathcal{F}=\emptyset$. For each $(R,F)\in \Nc$, if there is a pair $(C,k)\in\Cc$ such that $C\cap
        F=\emptyset$ and $k\neq 0$, then we add $C$ to $\mathcal{F}$ and update $m$ to be the maximum of
        its previous value and $k$; if there is no such pair, we add $R$ to $\Nc$.
        \item We output $\Pi'=(\Sigma\uout,\Nc',\Ec)$ and $k$. 
    \end{enumerate}
    This algorithm obviously terminates; we will prove that it is correct, by showing that
    every unsolvable instance of $\Pi$ corresponds to an unsolvable instance of $\Pi'$ up
    to appending
\end{proof}
...
\emph{Note: mostly here for me to have an overview and clickable common references; written as
if it was part of a paper as an exercise/research break activity.}

There is no doubt that LCL problems, defined by \cite{naor_what_1993}, form the most
extensively studied class of problems in the LOCAL model. Great progress happened mainly
in the last ten years, started from papers showing the presence of gaps \cite{changExponential2019}
\cite{chang_time_2017} and dense regions \cite{balliuNewClasses2018}. Now, the complexity landscape of LCL problems is almost completely known, in both the
deterministic and randomised setting; see \cite{suomelaSWATtalk2020}
for an overview. 

The LCL class is defined only on graphs with constant maximum degree and constant input
and output label size; this way, any LCL problem can be finitely described by the set of all
correctly labelled $r$-radius neighbourhoods. This property is useful for \emph{automated
algorithm synthesis}, that is, generating an optimal algorithm from the problem
description; this has been successful on paths, cycles [CITE?] and grids
\cite{brandtGrids2017}.

Given the above, we can describe an ideal generalisation of the LCL class as containing
problems that can be \emph{finitely described} and \emph{locally verified} by a
distributed algorithm. The largest possible class of verifiers that can be used is the
class of \emph{locally decidable} (LD) problems, as studied in \cite{fraigniaudComplexity2013}; we can then define the corresponding class of \emph{locally
verifiable} (LV) problems following \cite{balliuVerified2018} as problems whose verifier
is in LD. While this class is large and easily definable, it is hard to reason about in
terms of automated algorithm synthesis; algorithms, while finite, do not have a commonly used universal
notation that would allow for this kind of analysis. As an example, consider 2-colouring
and 3-colouring on trees; two problems that can be verified by two algorithms described
exactly the same way in pseudocode, but which have vastly different local complexities. 

A different approach was taken in [gustav's paper] with the class of \emph{locally
finite labelling} (LFL) problems, which have a structure similar to that of LCL problems
but allow for \emph{optional edges}. Thanks to this similarity, the gap theorem for the
$O(\log n)---\Omega(n)$ part of the complexity landscape of LCL problems on trees has been
shown to hold on LFL problems as well [cite same paper again]; however, this formulation
excludes many natural problems that are locally verifiable. 

In this work, we take a middle-ground approach: we describe problems through a subset of
first-order logic sentences called \emph{local formulas}, define the class of LFO (local
first order) problems, and show various results related to them. (\emph{Vague, but the
list is updating often.})
\clearpage
\subsection{Results}
Ever-changing list of results on LFO:
\begin{itemize}
    \item $LCL\subsetneq LFL\underset{?}{\subseteq} LFO \subsetneq LV$
    \item $0$-round solvability is decidable. (trying to prove it for $k$-round
    solvability for any constant $k$)
\end{itemize}
\subsection{Finite model theory notes}
\emph{Again mostly here as an exercise/reference, but parts of it would be useful/necessary for a 
proper write-up anyways. Not well formatted, for now.}

A \emph{vocabulary} is a set of symbols, each having an integer value called \emph{arity}.
Symbols with arity $=0$ are called \emph{constants}, otherwise \emph{relations}. A
\emph{structure} for the vocabulary $\tau$ is composed of a set $A$ called the universe,
an element $c^A$ of $A$ for every constant $c$ of $\tau$, and a $n$-ary relation $R^A$ on
$A$ for every relation $R$ of $\tau$ with arity $n$. A structure is finite if its universe
is finite.

A formula of first-order logic on a vocabulary $\tau$ is a string of symbols in the alphabet containing countably infinite
variables $v_1,\ldots,v_n$, the connectives $\lnot,\lor$, the quantifier $\exists$, the
equality symbol $=$, parenthesis symbols $(,)$, and the symbols in $\tau$ which is
well-formed, meaning it can be recursively created from the following rules:
\begin{enumerate}[(\roman*)]
    \item If $t_1,t_2$ are variables or constants (together called \emph{terms}), then
    $t_0=t_1$ is a formula.
    \item If $t_1,\ldots, t_n$ are terms and $R$ is a symbol of $\tau$ with arity $n$,
    then $Rt_1\ldots t_n$ is a formula.
    \item If $\phi$ is a formula, $\lnot\phi$ is a formula.
    \item If $\phi,\psi$ are formulas, then $(\phi\lor\psi)$ is a formula.
    \item If $\phi$ is a formula and $x$ is a variable then $\exists x \phi$ is a formula.
\end{enumerate}
If a formula is a result of only (i) and (ii), it is an \emph{atomic} formula. Denote by
FO$[\tau]$ the set of formulas of first-order logic on $\tau$. Commonly, we omit
parenthesis when they are understandable from context and use a list of auxiliary symbols:
\begin{itemize}
    \item $t_1\neq t_2$ for $\lnot(t_1=t_2)$,
    \item $\phi\land\psi$ for $\lnot(\lnot\phi\lor\lnot\psi)$,
    \item $\phi\implies\psi$ for $(\lnot\phi\lor\psi)$,
    \item $\phi\iff\psi$ for $(\phi\implies\psi)\land(\psi\implies\phi)$,
    \item $\forall x \phi$ for $\lnot(\exists x\lnot \phi)$, and
    \item for $R$ relation with arity $1$: $x\in R$ for $Rx$.
\end{itemize}


A variable is called \emph{bound} if it appears in the scope of a corresponding
quantifier, \emph{free} otherwise. A formula is called a \emph{sentence} if it has no free variables.
\clearpage
