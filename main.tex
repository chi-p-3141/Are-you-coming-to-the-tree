...
$\Pi=(\Delta,\Sigma\uin,\Sigma\uout,\Cc)$ where:
\begin{itemize}
    \item $\Delta$ is a natural number, representing the maximum degree of the graph class considered,
    \item $\Sigma\uin$ is a set of input labels,
    \item $\Sigma\uout$ is a set of output labels,
    \item $r$ is a finite integer, called the \emph{radius} of $\Pi$, and
    \item $\Cc$ is an indexed set $\Set{\Cc_{d,\chi}}_{d\leq\Delta,\chi\in\Sigma\uin}$
    where each $\Cc_{d,\chi}$ is a subset of $\Sigma\uout^{\MSet{d}}$ (representing the
...
\end{thm}
\begin{proof}
    Let $\Pi=(\Delta,\Sigma\uin,\Sigma\uout,\Cc)$. Let $\mathcal{T}^1_{\Delta}$ be the set of all
    ornaments of maximum degree $\Delta$: this is a countably infinite set, but
    $\mathfrak{Cl}(\mathcal{T}^1_{\Delta})$ is a finite set since it is a subset of
    $2^{\Sigma\uout}$. We want to classify the possible classes of ornaments for the
    problem $\Pi$ in three sets: classes that appear infinitely many times in the image of
    $\mathfrak{Cl}$, classes that appear finitely many times, and classes that never
    appear at all.
\end{proof}
\begin{thm}
